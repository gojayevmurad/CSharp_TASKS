    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            List<Debtor> debtors = new List<Debtor> {
                ...
                new Debtor("Pamela H. Beauch", DateTime.Parse("November 20, 1959"), "801-559-6347", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588)
            };

            //2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag

            //var resultTask2 = debtors.Where(s => s.Email.EndsWith("rhyta.com") || s.Email.EndsWith("dayrep.com"));
            ...
            //var resultTask3 = debtors
            //                .Where(d => DateTime.Now.Year - d.BirthDay.Year > 26 && DateTime.Now.Year - d.BirthDay.Year < 36);
            ...
            //5) Butov adi 15 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 1 reqemi olan borclulari cixartmaq

            //var resultTask5 = debtors.Where(s => s.FullName.Length > 15 && s.Phone.Count(c => c == '1') >= 2);
            ...
            //var averageDebts = debtors.Average(s => s.Debt);

            //var listSelectorDebtors = debtors
            //                        .Where(d => d.Debt > averageDebts);

            //var orderedListWithSurname = listSelectorDebtors.OrderBy(d => d.FullName);
            //var orderedListWithDebt = listSelectorDebtors.OrderByDescending(d => d.Debt);

            //foreach (var item in orderedListWithDebt)
            //{
            //    Console.WriteLine(item);
            //}
            ...
            //var resultTastk9 = debtors
            //                  .Where(d => d.Phone.Count(c => c == '8') == 0)
            //                  .Select(d => new { d.FullName, d.BirthDay, d.Debt });

            //foreach (var item in resultTastk9)
            //{
            //    Console.WriteLine(item);
            //}
            ...
            //var resultTask11 = debtors
            //    .Where(d => {
            //        for (int i = 0; i < d.FullName.Length; i++)
            //        {
            //            char currentChar = d.FullName[i];

            //            if (d.FullName.Count(c => c == currentChar) >= 3) return true;
            //        }
            //        return false;
            //    })
            //    .OrderBy(d => d.FullName);
            //foreach (var item in resultTask11)
            //{
            //    Console.WriteLine(item);
            //}
            ...
            //var rsultTast14 = debtors
            //    .OrderByDescending(d => d.Debt)
            //    .Take(5);

            //foreach (var item in rsultTast14)
            //{
            //    Console.WriteLine(item);
            //}
            ...
            //var resultTask16 = debtors
            //    .Where(d => d.BirthDay.Year <= 1945);

            //foreach ( var d in resultTask16)
            //{
            //    Console.WriteLine(d);
            //}
            ...
            //var resultTask18 = debtors
            //    .Where(d => d.Phone.Distinct().Count() == d.Phone.Count())
            //    .Sum(d => d.Debt);

            //Console.WriteLine(resultTask18);
            ...
            var resultTask19 = debtors
                .Where(d =>
                {
                    int totalPay = (12 - (DateTime.Now.Month - debtors[0].BirthDay.Month)) * 500;

                    if(totalPay > d.Debt) return true;
                    return false;
                });
